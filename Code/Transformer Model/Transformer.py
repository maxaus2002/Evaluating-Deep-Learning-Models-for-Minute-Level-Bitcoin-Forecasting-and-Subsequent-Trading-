"""
Minute-level Bitcoin price forecasting using a multi-horizon Transformer.

This script trains or loads a Transformer-based regression model to predict
future Bitcoin prices across multiple minute-ahead horizons. It advances
month by month: training on the previous three months and forecasting the
following month. For each forecast month it saves a CSV of predictions and
actuals, a performance plot, and appends summary metrics to a CSV.
"""
...
PLOT_LOG_SPACE = False
EMA_ENABLE = True
MASK_MEANS_WHERE_ACTUAL_EXISTS = False
RECON_MODE = "direct"
BIAS_ENABLE = False
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, d_model: int = 128, num_heads: int = 4, dropout: float = 0.2) -> None:
        super().__init__()
...
class TransformerMulti(nn.Module):
    """Transformer encoder with optional temporal pooling and a linear head.

    Predicts a vector of multi-horizon targets from a window of past features.

    Args:
        input_size: Number of input features per timestep.
        output_size: Number of forecast horizons.
        d_model: Embedding dimension.
        num_heads: Number of attention heads.
        num_layers: Number of encoder blocks.
        dropout: Dropout rate.
        pool_k: If > 1, apply 1D average pooling (stride = kernel) to reduce T.
    """
...
def _extract_gdrive_id(url_or_id: str) -> str:
    """Extract a Google Drive file ID from a URL or return if already an ID."""
...
def _gdrive_download_bytes(url_or_id: str) -> bytes:
    """Download file from Google Drive, handling confirmation page if needed."""
    file_id = _extract_gdrive_id(url_or_id)
    sess = requests.Session()
    r = sess.get("https://drive.google.com/uc", params={"export": "download", "id": file_id}, allow_redirects=True)
    r.raise_for_status()
    ...
    r2 = sess.get("https://drive.usercontent.google.com/download", params=params, allow_redirects=True)
    r2.raise_for_status()
    ct2 = r2.headers.get("Content-Type", "").lower()
    if "text/html" in ct2 and "attachment" not in r2.headers.get("Content-Disposition", "").lower():
        raise RuntimeError("Google Drive returned HTML (likely a confirm page). Ensure file is shared.")
    return r2.content

def load_data() -> pd.DataFrame:
    """Load price and fee data, align by minute, and engineer log-return features."""
    print("Loading data...")
    ...
    if pd.api.types.is_numeric_dtype(ts):
        ...
        if dt.isna().all():
            raise ValueError("Failed to parse datetimes from timestamp column.")
    ...
    if "Close" not in df.columns:
        raise KeyError("Missing 'Close' column after normalization.")
    ...
    if x_col is None or y_col is None:
        raise KeyError("Missing required columns in fee data.")
    ...
    fees_series = pd.Series(fees_df[y_col].values, index=fees_dt.dt.tz_convert(None)).rename("fee_usd").resample("1min").interpolate()
...
class BitcoinDataset(Dataset):
    ...
    Each item returns a window ``X[t-w:t]`` and the aligned target vector at ``t``.
    """
...
def train_model(
    ...
    """Train the model with early stopping and persist weights and scalers.

    Args:
        df_train: Training dataframe.
        features: Feature column names.
        target_cols: Target column names (log-returns per horizon).
        model_path: Destination for model weights.
        scaler_path: Destination for serialized scalers.
        label: Human-readable identifier for logs.
        max_epochs: Maximum training epochs.
        patience: Early-stopping patience on avg. training loss.
        batch_size: Batch size.
        pool_k: Temporal pooling factor.

    Returns:
        The trained model, feature scaler, and target scaler.
    """
    ...
    loader = DataLoader(ds, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True, persistent_workers=True, prefetch_factor=4, drop_last=True)
    model = TransformerMulti(input_size=len(features), output_size=len(target_cols), **BASE_MODEL_PARAMS, pool_k=pool_k).to(DEVICE)
    ...
    print(f"Saved model and scalers for {label}\n")
...
def compute_horizon_scale(
    ...
    """Estimate per-horizon scale as ``std(true_log) / std(pred_log)``.

    Computed over a calibration slice immediately preceding the forecast
    month. Values are clipped to ``[SCALE_MIN, SCALE_MAX]``.

    Args:
        df_slice: Calibration dataframe.
        model: Trained model.
        feat_scaler: Feature scaler.
        targ_scaler: Target scaler.
        features: Feature column names.
        target_cols: Target column names.
        window_size: Sliding window length.

    Returns:
        Array of per-horizon scale factors with shape ``[H]``.
    """
    X = feat_scaler.transform(df_slice[features])
    ds = BitcoinDataset(X.astype(np.float32), np.zeros((len(X), len(target_cols)), dtype=np.float32), window_size)
...
def compute_horizon_bias(
    ...
    """Estimate per-horizon bias as ``mean(pred_log - true_log)``.

    Computed over a calibration slice immediately preceding the forecast month.
    ...
    Returns:
        Array of per-horizon bias values with shape ``[H]``.
    """
    X = feat_scaler.transform(df_slice[features])
    ds = BitcoinDataset(X.astype(np.float32), np.zeros((len(X), len(target_cols)), dtype=np.float32), window_size)
    loader = DataLoader(ds, batch_size=512, shuffle=False, num_workers=0)
...
def smooth_forecast(arr: np.ndarray, alpha: float = 0.5, beta: float = 0) -> np.ndarray:
    """Apply a causal EMA with optional pull toward the causal running mean.

    Args:
        arr: Input series.
        alpha: EMA weight on the current point in ``[0, 1]``.
        beta:  Strength of pull toward the causal running mean in ``[0, 1]``.
    ...
    out, running_mean = np.empty_like(x), x[0]
    out[0] = x[0]
...
def forecast_month(
    ...
    Applies optional bias/scale calibration and EMA smoothing. Returns a
    dataframe containing forecasts, actuals, and auxiliary fields.

    Args:
        df_period: Target month dataframe.
        model: Trained model.
        feat_scaler: Feature scaler.
        targ_scaler: Target scaler.
        features: Feature column names.
        ...
        pool_k: Temporal pooling factor.
        bias: Optional per-horizon bias correction (log-space).
        scale: Optional per-horizon scale factor (log-space).

    Returns:
        DataFrame with columns ``forecast_t+{h}m``, actuals, and metadata.
    """
    ...
    if DEVICE.type == "cuda":
        torch.cuda.empty_cache()
    unp_log = targ_scaler.inverse_transform(np.vstack(preds_log))
    ...
    df_out["period"] = label
    for h in HORIZONS:
        df_out[f"actual_t+{h}m"] = (df_period["Close"].shift(-h).iloc[WINDOW_SIZE - 1 : WINDOW_SIZE - 1 + len(unp_log)].values)
...
if __name__ == "__main__":
    ...
    for i in range(len(periods) - 3):
        ...
        csv_path = os.path.join(OUTPUT_DIR, str(fore_month.year), fore_month.strftime("%B_%Y"), f"forecast_{fore_month.strftime('%B_%Y')}.csv")
        csv_path_Yearly = os.path.join(OUTPUT_DIR_YEARLY, str(fore_month.year), fore_month.strftime("%B_%Y"), f"forecast_{fore_month.strftime('%B_%Y')}.csv")
        if os.path.exists(csv_path):
            train_months = periods[i : i + 3]
            label = (f"{train_months[0].strftime('%b %Y')}+{train_months[1].strftime('%b %Y')}+{train_months[2].strftime('%b %Y')} -> {fore_month.strftime('%b %Y')}")
        ...
        if os.path.exists(model_path):
            model = TransformerMulti(input_size=len(features), output_size=len(target_cols), **BASE_MODEL_PARAMS, pool_k=12).to(DEVICE)
            ...
            bias = None
            if BIAS_ENABLE:
                bias = compute_horizon_bias(df_train, model, feat_scaler, targ_scaler, features, target_cols, WINDOW_SIZE)
            scale = None
            if SCALE_ENABLE:
                scale = compute_horizon_scale(df_train, model, feat_scaler, targ_scaler, features, target_cols, WINDOW_SIZE)
        else:
            df_train = df[df.index.to_period("M").isin(train_months)]
            model, feat_scaler, targ_scaler = train_model(df_train, features, target_cols, model_path, scaler_path, label, max_epochs=15, patience=3, batch_size=256, pool_k=12)
            bias = None
            if BIAS_ENABLE:
                bias = compute_horizon_bias(df_train, model, feat_scaler, targ_scaler, features, target_cols, WINDOW_SIZE)
            scale = None
        ...
        df_out = forecast_month(df_test, model, feat_scaler, targ_scaler, features, target_cols, label, pool_k=12, bias=bias, scale=scale)
        ...
        pd.DataFrame([{"period": fore_month.strftime("%B %Y"), "MSE": mse, "RMSE": rmse, "NRMSE": nrmse, "MAPE (%)": mape}]).to_csv(metrics_csv, mode="a", index=False, header=False)
        pd.DataFrame([{"period": fore_month.strftime("%B %Y"), "MSE": mse, "RMSE": rmse, "NRMSE": nrmse, "MAPE (%)": mape}]).to_csv(metrics_csv_Yearly, mode="a", index=False, header=False)
    mdf = pd.read_csv(metrics_csv)
    o_mse, o_rmse, o_nrmse, o_mape = (mdf["MSE"].mean(), mdf["RMSE"].mean(), mdf["NRMSE"].mean(), mdf["MAPE (%)"].mean())
    print(f"Overall avg MSE: {o_mse:.6f}, RMSE: {o_rmse:.6f}, NRMSE: {o_nrmse:.6f}, MAPE: {o_mape:.2f}%")
    pd.DataFrame([{"period": "Overall", "MSE": o_mse, "RMSE": o_rmse, "NRMSE": o_nrmse, "MAPE (%)": o_mape}]).to_csv(metrics_csv, mode="a", index=False, header=False)
    pd.DataFrame([{"period": "Overall", "MSE": o_mse, "RMSE": o_rmse, "NRMSE": o_nrmse, "MAPE (%)": o_mape}]).to_csv(metrics_csv_Yearly, mode="a", index=False, header=False)
