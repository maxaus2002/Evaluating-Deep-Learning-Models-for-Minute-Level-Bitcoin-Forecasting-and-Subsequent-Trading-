...
FORECAST_ROOT = "monthly_forecasts"
OUTPUT_ROOT = "monthly_results"
...
# =============================================================================
PARAMETER_GRID = {
    "short_gain_mult": [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2],
    "mid_gain_mult": [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2],
    "long_gain_mult": [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2],
    "bullish_slope_short": [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2],
    "bullish_slope_mid": [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2],
    "bullish_slope_long": [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2],
    "short_gain_mult_sell": [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2],
    "mid_gain_mult_sell": [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2],
    "long_gain_mult_sell": [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2],
    "bearish_slope_short": [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2],
    "bearish_slope_mid": [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2],
    "bearish_slope_long": [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2],
...
def _load_champion(champion_csv_path: str) -> dict | None:
    """Load the best-parameter row from CSV, if present."""
...
def evaluate_params(params_arr: np.ndarray,
                    ...
                    mp: np.ndarray,
                    lp: np.ndarray) -> float:
    """Simulate one month with a parameter array and return terminal value."""
    cash = START_CASH
    btc = 0.0
    n = len(closes)
...
def run_grid_search(csv_path: str, config_path: str, n_iter: int = 3000, random_state: int = 50):
    ...
    out_dir = os.path.join(os.getcwd(), OUTPUT_ROOT, rel)
...
def simulate_history_fast(df: pd.DataFrame, params: dict, forecast_cols: list[str], START_CASH: float = START_CASH) -> pd.DataFrame:
    """Vectorized month simulation; returns portfolio trajectory and fees."""
    idx = df.index
...
def simulate_month_pair(pair: tuple[str, str]) -> None:
    ...
    forecast_root_abs = os.path.join(base, FORECAST_ROOT)
    ...
    history = simulate_history_fast(df1, chosen_params, forecast_cols, START_CASH=START_CASH)
...
if __name__ == "__main__":
    base = os.path.dirname(os.path.abspath(__file__))
    forecast_root = os.path.join(base, FORECAST_ROOT)
    if not os.path.isdir(forecast_root):
        raise FileNotFoundError(f"Missing {FORECAST_ROOT}")

    months = month_dirs(forecast_root)
    pairs = [(d0, d1) for (_, _, d0), (_, _, d1) in zip(months, months[1:])]
    for pair in pairs:
        simulate_month_pair(pair)
